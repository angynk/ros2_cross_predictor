...
def show_seg_result(img, result, palette=None,is_demo=False):
    ...
    #img = cv2.resize(img.copy(), (1280,720), interpolation=cv2.INTER_LINEAR)
...
def driving_area_mask(scale,seg = None):
    da_predict = seg[:, :, 12:372,:]
    da_seg_mask = torch.nn.functional.interpolate(da_predict, scale_factor=scale, mode='bilinear')
    _, da_seg_mask = torch.max(da_seg_mask, 1)
    da_seg_mask = da_seg_mask.int().squeeze().cpu().numpy()
    return da_seg_mask

def lane_line_mask(scale,ll = None):
    ll_predict = ll[:, :, 12:372,:]
    ll_seg_mask = torch.nn.functional.interpolate(ll_predict, scale_factor=scale, mode='bilinear') # Scale factor 2 : 720 X 1280 , 3: 1080 X 1920
